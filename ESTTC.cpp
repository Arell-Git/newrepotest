...
#ifdef DEBUG_ENABLED
/* Define this symbol to echo over DEBUG_COMM every character ESTTC receives over any interface. */
//#define ECHO_CHAR
/* Define this symbol to echo over DEBUG_COMM every complete phrase ESTTC receives over any interface. */
//#define ECHO_PHRASE
#endif
...
static const FILE * Esttc_usart_interfaces[ESTTC_INTERFACE_NUMBER] = {COM1, COM6};
static uint32_t fpos[ESTTC_INTERFACE_NUMBER], bsize[ESTTC_INTERFACE_NUMBER];
static FILINFO fno[ESTTC_INTERFACE_NUMBER];
static uint16_t pack_data_position[ESTTC_INTERFACE_NUMBER] = {0,0};
...
void ESTTC_InitTask(void)
{
    ...
    if( HAL_OK != retVal ){
        panic("%s", "Fail HAL_UART_Receive_IT");
    }
/*
...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
*/
void ESTTC_UART_TASK(void * argument)
{
...
{
  fprintf(f, "OBC BOOT\n");
  fprintf(f, "SW version: Appl v.%d.%02d compiled at <%s %s>\r\n",
...
#ifdef DEBUG_ENABLED
      debugCmdParse(rxline[Interface], DEBUG_COMM);
#endif
      ...
      if(begin != NULL)
      {
        ...
        if( len <= LINE_BUFFER_SIZE )
        {
          ...
          if (txline[Interface][3] == 'R')
          {
            ...
            /* UHF ANTENNA ESTTC DISABLED since OBC isn't connected to antenna anyways
            else
            ...
            UHF ANTENNA ESTTC DISABLED */
            else if (txline[Interface][4] == OBC_I2C_ADDRESS)
            {

              switch(txline[Interface][5])
              {
              /* ACCEL ESTTC DISABLED since we're not using it
                ...
                case ESTTC_CMD_GET_TIME: //0x31:   // get time
                  ...
                  vTaskDelay(pdMS_TO_TICKS(1));/* wait time to copy the data to shadow registers */
              ...
              if (txline[Interface][5] <= ESTTC_CMD_PAR_NUM)
              {
                  ...
                  case ESTTC_CMD_MGA1_ACESS: //0x05:
                    ...
                    case ESTTC_CMD_RST_COUNTS:  // 0x61
                    {
                        ...
                        memcpy(&EEPROM_emul_DataTemp.ResetFaults, (void *)BootData, sizeof(EEPROM_emul_DataTemp.ResetFaults));
                        // YUAA: TODO: why is the size of this copy sizeof(EEPROM_emul_DataTemp)? 
                        // Shouldn't it be sizeof(boot_struct)?
...
#endif
                      vTaskDelay(pdMS_TO_TICKS(50));             /* wait some time until the USART buffers are empty */
                      // Reset the OBC
...
#ifdef ECHO_PHRASE
          fprintf(DEBUG_COMM, "ESTTC ECHO: %s\r\n", dst);
#endif
          return 1;
      }
  }
